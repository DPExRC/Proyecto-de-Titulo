...
#   - 2 DOF: codo (bidireccional) + hombro (unidireccional), reposo = 0°
...
#   bic=67.3  tri=8.1  delt=22.4  →  codo=142.3°  hombro=21.0°
#
...
def hilo_control(bridge: SerialBridge, predictor: EMGPredictor):
    ...
    while flag_activo.is_set():
        ...
        angulo_hombro = resultado["angulo_hombro"]

        # Enviar ambos ángulos al Arduino
        bridge.enviar_angulos(angulo_codo, angulo_hombro)

        # Log cuando algún ángulo varía significativamente
        cambio_codo   = abs(angulo_codo   - angulo_codo_ant)   > LOG_UMBRAL_CAMBIO
        cambio_hombro = abs(angulo_hombro - angulo_hombro_ant) > LOG_UMBRAL_CAMBIO

        if cambio_codo or cambio_hombro:
            bic  = features[_IDX_RMS_BICEPS]
            tri  = features[_IDX_RMS_TRICEPS]
            delt = features[_IDX_RMS_DELTOIDES]
            print(
                f"  bic={bic:6.1f}  tri={tri:6.1f}  delt={delt:6.1f}"
                f"  →  codo={angulo_codo:6.1f}°  hombro={angulo_hombro:6.1f}°"
            )
            angulo_codo_ant   = angulo_codo
            angulo_hombro_ant = angulo_hombro
...
def main():
    ...
    print("  Canales: bíceps, tríceps, deltoides")
    print("  DOF: codo (bidireccional) + hombro (unidireccional)")
